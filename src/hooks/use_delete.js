 ...
 * }
 */
export default function useDelete(timeout = NetworkConstants.TIMEOUT) {
  ...
  const deleteIt = async (url, params = {}, headers = {}) =>  {
    try {
      const res = await Api.del(url, settings.api_url, jwt, settings.csrf_token, params, headers, timeout);
      setResult(res);
    } catch (err) {
      setError(err);
    }
